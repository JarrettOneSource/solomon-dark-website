...
// ambient songs out of it, and each visit here picks one at random. One ♪
// toggle governs the music and the UI ticks together; nothing plays before
// the first user gesture (autoplay policy — and manners).
...
function startMusic(track = TRACKS[Math.floor(Math.random() * TRACKS.length)]) {
  ...
  const target = () => (document.hidden ? 0.02 : MUSIC_VOLUME)
  void el.play().then(() => fadeTo(target(), 2500)).catch(() => {
...
// Duck while the tab is away — the class continues, but quietly.
document.addEventListener('visibilitychange', () => {
  if (!audio || isMuted()) return
  fadeTo(document.hidden ? 0.02 : MUSIC_VOLUME, 900)
})
