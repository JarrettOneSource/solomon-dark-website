...
export function nativeEnemyProjectileLightProvider(
  projectile: NativeEnemyProjectileLightOwner,
  presentationFrame: number,
  multipleShadows = NATIVE_DEFAULT_MULTIPLE_SHADOWS,
...
export function nativeEnemyProjectileEffectLightProvider(
  effect: NativeEnemyProjectileEffectLightOwner,
  multipleShadows = NATIVE_DEFAULT_MULTIPLE_SHADOWS,
  pointGain = 1,
): NativeBoneyardLightProviderCandidate | null {
...
export function nativeRegionLightStamp(
  ...
  screenPosition: Vec2,
  sprite: { anchorX: number; anchorY: number; h: number; w: number },
  zoom: number,
): {
  alpha: number
  anchorX: number
...
function nativeFireProjectileLightSource(
  projectile: Pick<NativeEnemyProjectileLightOwner, 'id' | 'position'>,
  presentationFrame: number,
): NativeBoneyardLightSource {
  return {
    castsDirectionalShadow: false,
...
interface NativeEnemyProjectileLightOwner {
  readonly id: number
  readonly kind: 'arrow' | 'firebolt' | 'guided-missile' | 'demon-bomb' | 'poison-pool'
  readonly payload: string
  readonly position: Vec2
}

interface NativeEnemyProjectileEffectLightOwner {
