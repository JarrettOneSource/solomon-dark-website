...
test('secondary belt keeps the exact eight 53 px slots and 60 px pitch', () => {
  assert.deepEqual(NATIVE_SECONDARY_BELT_SLOT_OFFSETS, [
    -332, -272, -212, -152, 98, 158, 218, 278,
  ])
  assert.deepEqual(
    NATIVE_SECONDARY_BELT_SLOT_OFFSETS.slice(1, 4).map((offset, index) => (
      offset - NATIVE_SECONDARY_BELT_SLOT_OFFSETS[index]!
    )),
    [60, 60, 60],
  )
})
...
test('secondary belt lays out native group-8 key labels over 13 px plaques', () => {
  assert.equal(NATIVE_SECONDARY_BELT_FONT.group, 8)
  assert.deepEqual(NATIVE_SECONDARY_BELT_FONT.header, [10, 3, 28])
  assert.equal(NATIVE_SECONDARY_BELT_FONT.glyphCount, 92)
  assert.equal(NATIVE_SECONDARY_BELT_FONT.kerningCount, 1)
  assert.equal(NATIVE_SECONDARY_BELT_FONT.scale, 1)

  assert.deepEqual(layoutNativeSecondaryBinding('3'), {
    advance: 7,
    backingLeft: 20,
    backingWidth: 13,
    glyphs: [{
  ...
  assert.deepEqual(layoutNativeSecondaryBinding('4').glyphs[0], {
    atlasX: 435,
    atlasY: 9,
    char: '4',
    height: 9,
