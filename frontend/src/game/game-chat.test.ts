...
test('whisper UX runs from the Player Card into a dedicated chat thread', () => {
  ...
  assert.match(playerCard, /onMessage\(playerId, displayName\)/)
  assert.match(hubScene, /onMessage=\{onMessagePlayer\}/)
  assert.match(boneyardScene, /onMessage=\{onMessagePlayer\}/)
