...
test('packed Boneyard batches isolate worlds and preserve schema-v7 strides', () => {
  ...
  assert.equal(observations.length, 2 * 3_026)
  assert.equal(transition.observations.length, 2 * 3_026)
  ...
  assert.equal(transition.gameplayCounters.length, 2)
  assert.ok(transition.gameplayCounters.every(({ enemyKills }) => enemyKills >= 0))
