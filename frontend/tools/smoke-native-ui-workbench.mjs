...
try {
  ...
  const stoneAction = dom.locator('[data-native-ui-stone-button]')
  assert.equal(await stoneAction.count(), 1)
  assert.equal(await stoneAction.locator('[data-native-ui-record="UI.105"]').count(), 1)
  await stoneAction.dispatchEvent('pointerdown', { button: 0, pointerType: 'mouse' })
