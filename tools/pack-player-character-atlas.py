...
def pack_rectangles(rectangles: list[PackedRectangle]) -> list[Image.Image]:
    ...
    pages = [
        Image.new("RGBA", (PAGE_SIZE, max(1, sum(shelf.height for shelf in shelves))))
        for shelves in shelves_by_page
    ]
...
def generated_module(
    ...
    page_names = ", ".join(f"page{index}" for index in range(page_count))
...
def main() -> None:
    ...
    if len(paths) != 100:
        raise SystemExit(f"expected 100 player source sheets, found {len(paths)}")
    sheets, rectangles = collect_sources(paths)
    pages = pack_rectangles(rectangles)
    if len(pages) > 3:
        raise SystemExit(f"player frames require {len(pages)} atlas pages; expected at most 3")
    ...
    write_or_check(
        module_path,
        generated_module(sheets, rectangles, pages),
        args.check,
    )
    print(
