...
- September 4 correction: Flash follows Deflect, resistance, shield/stone
  interception, status admission, health processing and ouch. The receiver
  must reach its final response branch with tertiary poison damage zero and
  damage flag `0x40` clear. It executes
  `d=Integer(100); d>0 && d<=trunc(mChance)`. Roll zero fails. Lightning's
  `0x42` flags prevent Flash; armor can reduce health damage to zero without
  suppressing this final response branch.
- Success consumes `Float(.2)` for `flashspell` playback rate `1+draw`, one
  ...
  child-scale draws. These response words follow any earlier Deflect and ouch
  draws. A successful Deflect returns before Flash.
...
response RNG at the receiver's final response branch, then materializes one semantic response event,
...
Focused coverage pins percentile zero, successful chance, eleven RNG words for
admitted Flash and three for successful Deflect, radius membership, Dazzle merge, 8+4 actor
