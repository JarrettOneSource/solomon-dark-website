...
| Portal presentation/light | main `0x004A1B30`; alternate `0x004A1CB0`; light provider `0x0047BED0`; DeadHawg builder map | `+0x220` grows by `.025` to one. `+0x214` advances by `.15+U(.15)` over DeadHawg 46..77; `+0x210` advances by `.05+U(.2)` over 180..199; fixed scale is `1.25+U(.5)`. The alternate pass also consumes inline DeadHawg 18 and 22. Corrected 2026-09-07: light intensity is live `+0x220`; radius is `f32(+0x220*f32(0.8999999761581421+Float(0.3500000238418579)))`. Multiple Shadows owns the containment flag. | high |
