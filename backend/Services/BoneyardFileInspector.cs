...
internal static class BoneyardFileInspector
{
    ...
    public static void Validate(Stream stream, string label)
    {
        ...
        {
            throw Invalid(label, reader.Offset, "the SyncBuffer is truncated");
        }
        catch (OverflowException)
        {
            throw Invalid(label, reader.Offset, "a SyncBuffer length overflows");
        }
    }
        ...
        int depth)
    {
        if (depth > MaxDepth)
        {
            throw new ModPackageValidationException(
                $"Boneyard SyncBuffer nesting exceeds {MaxDepth} levels.");
        }

        if (state.ChunkCount == MaxChunks)
        {
            throw new ModPackageValidationException(
                $"Boneyard SyncBuffer contains more than {MaxChunks} chunks.");
        }
    ...
    private static ModPackageValidationException Invalid(
        string label,
        long offset,
        string reason) =>
        new($"Boneyard '{label}' is invalid at byte {offset}: {reason}.");
    ...
    private sealed class CountingReader(Stream stream)
    {
        ...
        private void EnsureFileLimit()
        {
            ...
            {
                throw new ModPackageValidationException(
                    "Boneyard files may not exceed 256 MiB.");
            }
        }
    }
}
